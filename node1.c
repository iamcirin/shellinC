 ...
 */
list_t *add_node(list_t **list_head, const char *s, int num)
{
	list_t *new_head;
	...
	if (!new_head)
		return (NULL);
	alloc_mem((void *)new_head, 0, sizeof(list_t));
	new_head->num = num;
	if (s)
	{
		new_head->str = string_dup(s);
		if (!new_head->str)
		{
			free(new_head);
			return (NULL);
 ...
 * Return: size of list
 */
list_t *add_node_end(list_t **list_head, const char *str, int num)
{
	...
	if (!new_node)
		return (NULL);
	alloc_mem((void *)new_node, 0, sizeof(list_t));
	new_node->num = num;
	if (str)
	{
...
/**
 * print_list_str - a function that prints only the str element of a list_t
