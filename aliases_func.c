...
int unset_alias(info_t *data_list, char *str)
{
	...
	*ptr = 0;
	ret =
	delete_node_at_index(&(data_list->alias),
		get_node_index(data_list->alias,
		node_starts_with(data_list->alias, str, -1)));
	*ptr = c;
	return (ret);
}
...
int set_alias(info_t *data_list, char *str)
{
	...
	return (add_node_end(&(data_list->alias), str, 0) == NULL);
}
...
int _myalias(info_t *data_list)
{
	...
	for (i = 1; data_list->argv[i]; i++)
	{
		...
		if (ptr)
			...
			print_alias(node_starts_with(data_list->alias, data_list->argv[i], '='));
	}
